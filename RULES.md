...
## 27. Minimal Retrieval Layer ("RAG") Rules (CEO Decision, 2026-09-13 Asia/Calcutta)
- **Reviewed-source-only corpus.** The retrieval corpus (`docs/automation/rag-corpus/sources.json`) may contain only excerpts from documents already marked "Reviewed" in `docs/SOURCES.md`. No unreviewed URL, informal source, or general web content may be added to this corpus at any time.
- **No user/log data ingestion — ever.** Applicant messages, Telegram chat IDs, Google Sheet log rows, reminder text, free-text input, credentials, or any derivative of them must never be written into the retrieval corpus or any embeddings/vector index. There is no ingestion pipeline for such data by design (`Architecture.md` §29.3 item 5) — this is a structural absence, not a filter that could be bypassed.
- **No unreviewed retrieval.** If retrieval matches zero entries, matches weakly/ambiguously, or the request needs a district-level hospital/medical-board/welfare-office fact not present in any Reviewed entry's `allowed_claims`, the workflow must return the existing fixed fail-safe (`docs/automation/D1.3a-workflow-spec.md` §5) rather than let Gemini guess or generalize.
- **No new external infrastructure without a fresh CEO decision.** Retrieval must not silently grow into a paid vector database, a new SaaS integration, a web scraper, or an autonomous multi-step agent. The current design (`Architecture.md` §29) is deliberately a static, deterministic filter over 5 curated entries — not embeddings-based similarity search — because this n8n instance has no embeddings node installed (verified 2026-09-13) and the corpus is too small to need one regardless.
- **Manual re-sync discipline.** `rag-corpus/sources.json` must be manually updated whenever `docs/SOURCES.md` changes; there is no automated sync job, and claiming the two are in sync without checking is a documentation-accuracy violation (`RULES.md` §15 Evidence Requirements).
