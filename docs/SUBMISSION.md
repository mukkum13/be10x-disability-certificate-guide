...
**Outreach status update (2026-09-17):** A consultation request was sent by the CEO/Product Owner to a social-worker contact via WhatsApp on 2026-09-17 at 3:28 PM IST. No name or phone number is recorded here, per privacy preference. **Status: response received — see "Controlled trial feedback" entry immediately below.** The contact tried the live Telegram bot directly and gave brief written feedback through the bot itself at approximately 3:53 PM IST the same day. This is informal written trial feedback, not a formal expert approval or a completed independent-review checklist (see below).
...
- **Telegram bot (primary verified product channel):** n8n workflow `gXPlaislJyNuqJSd` ("My workflow 2") — 11 nodes (Telegram Trigger → Normalize Telegram Update → Escalation-Keyword Interrupt → [true] Escalation Response / [false] Reminder Keyword Check → [true] Reminder Wait 2 min → Send Reminder / [false] Grounded Response Agent with Google Gemini Chat Model + Window Buffer Memory sub-nodes → Output Delivery). **Live-verified** end-to-end: five-question guided flow, source-grounded final answer, escalation fallback, mid-conversation language switch (2026-09-16); exact-match "remind me" reminder with real 2-minute-delayed Telegram delivery (2026-09-17, n8n execution #23); one real informal trial by a social-worker contact (2026-09-17). Public link: `https://t.me/MUKKUM13_BOT`. The workflow is kept **unpublished** between authorized, controlled tests, per standing safety procedure.
