...
## Node 1a — Normalize Telegram Update
- **Purpose:** Standardize incoming Telegram updates so only the in-flight routing fields pass downstream and no raw Telegram payload fields can reach later nodes.
- **Input:** The Telegram Trigger output object, including either a message or callback_query payload.
- **Output:** Exactly three fields: `session_key`, `inbound_value`, and `inbound_kind`.
- **Approved data fields carried forward:**
...
- **Prohibited data fields:** no raw Telegram payload fields are retained or forwarded beyond these three in-flight fields.
- **Branch conditions:** none; this node is a normalization-only gate inserted between the Telegram Trigger and Node 1b.
- **Failure behaviour:** if no message or callback payload is available, the node still emits the three fields with safe empty/default values rather than passing raw Telegram data downstream.
- **Source-grounding rule:** N/A.
- **Unpublished/disabled state requirement:** same as Node 1.
- **Current build status (live-verified 2026-09-14 16:56 Asia/Calcutta):** this node exists between the Telegram Trigger and Node 1b in the unpublished draft. It is an Edit Fields/Set node in Manual Mapping mode with exactly the three approved fields, and **Include Other Input Fields** is disabled. After a full workflow reload, Node 1b, Node 2a, Node 2b, and Node 2c were each confirmed to read only the normalized `inbound_value` or `session_key` fields specified in this manifest. No execution occurred.
...
- **Source-grounding rule:** N/A.
- **Unpublished/disabled state requirement:** same as Node 1.
- **Current build status (truthful, live-verified 2026-09-14 16:56 Asia/Calcutta):** this node exists in the Product Owner's n8n workflow (`Sln6CeMzfuO1ezru`), immediately downstream of Node 1a. It is configured with **Must Match = Any Condition (OR)** and exactly four case-insensitive `contains` conditions — each left side is `{{ $json.inbound_value }}`; fixed values are `stuck`, `help`, `problem`, and `not working`. After a full editor reload, all four left-side expressions were visibly reconfirmed. Its `false` output fans out to Node 2a and Node 2b; its `true` output remains unconnected. No credential entered; nothing executed, published, or activated.
...
- **Implementation — a separate, temporary n8n Data Table (CEO-decided, not an AI Agent "Simple Memory" sub-node and not workflow static data):**
  ...
  - **Lifecycle — 30-minute session expiry (confirmed in the live `Insert Row` initializer, 2026-09-14 11:51 Asia/Calcutta):** each new session row's `expires_at` is set to exactly 30 minutes after `updated_at` at creation time (`{{ $now.plus({ minutes: 30 }).toISO() }}` against `{{ $now.toISO() }}`), live-verified via the node's own expression preview (e.g. `updated_at` → `2026-09-14T12:00:37...`, `expires_at` → `2026-09-14T12:31:02...`, exactly 30 minutes apart). The session row is deleted once the final response/reminder flow for that conversation completes. A separate **expired-session cleanup path** (a scheduled/triggered node checking `expires_at` against current time and deleting stale rows) must exist before this table is relied upon in a live flow — not yet built; specified here only.
  - **Design correction — paired single-output gates, not a true/false branch (CEO Decision, 2026-09-14 11:43 Asia/Calcutta):** the Data Table node's `If Row Exists` and `If Row Does Not Exist` actions are **not** a branching pair like the core `If` node's true/false outputs. Each is a single-output action that either produces an item (condition met) or produces nothing (condition not met) — confirmed by DOM inspection showing exactly one unlabeled output handle per Data Table node, versus the core `If` node's two labeled `true`/`false` handles. The session-exists/does-not-exist check is therefore implemented as **two separate, single-output gate nodes, both fed from the same upstream `false` output of Node 1b** (n8n allows one output to fan out to multiple downstream nodes) — not as two branches of one node.
  - **Build status (truthful, verified 2026-09-14 11:30–11:45 Asia/Calcutta):** `UDID_Guide_Sessions` **exists** in the Product Owner's n8n instance (ID `X8Xhx2Qwvs4R0Tx9`), with all 9 approved custom string columns and the expected n8n defaults (`id`, `createdAt`, `updatedAt`), and **zero rows**. **Two gate nodes now read it, both fed from Node 1b's `false` output** (confirmed via each edge's DOM id, `.../outputs/main/1`, where index 1 is the `false` branch — the only branching output on the canvas):
    - **`Node 2a — Session Exists`** — `Data table` node, `If Row Exists` operation, one condition: Column `session_key`, Value (expression) `{{ $json.session_key }}`. Produces an item only when a session row already exists for this chat.
    - **`Node 2b — Session Does Not Exist`** — `Data table` node, `If Row Does Not Exist` operation, one condition: Column `session_key`, Value (expression) `{{ $json.session_key }}`. Produces an item only when no session row exists for this chat.
    - Neither action exposes a separate operator field — equality is each action's only available matching behaviour, satisfying the CEO's "Equals"/condition requirement implicitly rather than as a separately configured setting.
    - Node 1b's `true` output and both Node 2a's and Node 2b's own outputs remain unconnected. No credential entered; nothing executed, published, or activated; `UDID_Guide_Sessions` confirmed still at 0 rows after this step; `UDID_Reviewed_Sources` untouched.
  - **New-session initializer built (2026-09-14 11:51–12:02 Asia/Calcutta):** **`Node 2c — Initialize Session`** — `Data table` node, `Insert row` operation, `UDID_Guide_Sessions` selected, "Map Each Column Manually," with exactly the 9 approved columns populated: `session_key` (expression `{{ $json.session_key }}`), `current_question_index` (fixed `"0"`), `state_district`/`disability_type`/`applicant_relationship`/`udid_status`/`language` (fixed, left empty), `updated_at` (expression `{{ $now.toISO() }}`), `expires_at` (expression `{{ $now.plus({ minutes: 30 }).toISO() }}`). Connected only from `Node 2b — Session Does Not Exist`'s output (confirmed via edge DOM id `d9efaaaf.../outputs/main/0` → `a779414b.../inputs/main/0`). Node 2c's own output remains unconnected. No credential entered; nothing executed, published, or activated; `UDID_Guide_Sessions` confirmed still at 0 rows after this step; Node 2a, Node 1b, Node 4.5, and `UDID_Reviewed_Sources` untouched.
  - **Not yet built:** the `Insert Row` session initializer (to run only from Node 2b's output once wired) — specified in a prior design pass but not yet created, per explicit CEO instruction.
...
- **Unpublished/disabled state requirement:** same as Node 1.
- **Build status (truthful, verified 2026-09-14 12:10–12:20 Asia/Calcutta):** **`Node 3a — Ask Q1 State`** exists on the canvas — a Telegram node, "Send a text message" action (Resource: Message, Operation: Send Message), connected only from `Node 2c — Initialize Session`'s output (confirmed via edge DOM id `a779414b.../outputs/main/0` → `480f036a.../inputs/main/0`). Configured: Chat ID = expression `{{ $json.session_key }}`; Text (fixed) = `"Before we begin: please select your State or Union Territory. This guide uses reviewed official sources and does not make medical or legal decisions."`. **No credential selected** — the node shows the same "missing credential" warning icon as the pre-existing Telegram Trigger, confirming the credential selector was left empty as instructed. This is a **plain text prompt only** — it does not yet implement the closed-choice inline keyboard specified above for Q1; the keyboard, district follow-up, callback handling, session update, and all downstream nodes remain unbuilt, per explicit CEO scope. Node 3a's own output remains unconnected. Nothing executed, published, or activated; no Telegram message sent.
