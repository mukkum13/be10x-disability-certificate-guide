...
Per `Architecture.md` §28, Telegram and the website are two front ends to one shared workflow — this section governs how that shows up in copy/UI, not just backend logic:
- **Single copy source.** The disclosure banner text, the five questions' exact wording, the numbered-step labels ("what to do"/"where to do it"/"what to carry"/"what happens next"), the fail-safe sentence, the escalation placeholder, and the mandatory closing line are written **once** (in this file / the n8n prompt) and referenced by both channels — never re-typed separately for Telegram vs. the website, to prevent silent drift.
- **Telegram-specific presentation only:** plain-text formatting instead of styled cards/buttons where Telegram's UI requires it (e.g., numbered list as plain text, "Reply STUCK" instead of a button) — the wording itself must still match the website exactly.
- **Website-specific presentation only:** the visual layout, buttons, and progress bar already built in the Lovable shell (`MEMORY.MD` Entry 013) — again, wording must match Telegram's, not diverge.
- **Parity check as a release gate:** before either channel is marked complete, run the same test input through both and confirm byte-identical guidance text (aside from Telegram/website-only formatting noted above). Any mismatch is a defect, logged in `MEMORY.MD`, not silently picked one way.
